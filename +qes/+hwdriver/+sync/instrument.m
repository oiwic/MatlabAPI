...
% mail4ywu@gmail.com/mail4ywu@icloud.com
    ...
    methods
        function obj = instrument(name,interfaceobj,drivertype)
            if ~isa(interfaceobj,'icinterface') && ~isa(interfaceobj,'qes.hwdriver.sync.ustcda')...
                    && ~isa(interfaceobj,'qes.hwdriver.sync.ustcad') && ~isa(interfaceobj,'qes.hwdriver.sync.ustcadda')
...
%                     end
%                 case '()'
%                     objs = qes.qHandle.FindByProp('name',S(1).subs{1});
%                     removeidx = [];
%                     for ii = 1:numel(objs)
%                         if ~isa(objs{ii},'instrument');
