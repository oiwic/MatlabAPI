classdef SpectrumAnalyzer < Instrument
    % SpectrumAnalyzer source driver, basic.
    % basic properties and functions of a SpectrumAnalyzer source, for extensive
    % properties and functions, use class SpectrumAnalyzer_e.
    ...
    methods (Access = private)
        function obj = SpectrumAnalyzer(name,interfaceobj,drivertype)
            if isempty(interfaceobj)
                error('SpectrumAnalyzer:InvalidInput',...
            ...
            if ~isempty(ErrMsg)
                error('SpectrumAnalyzer:InstSetError',[obj.name, ': %s'], ErrMsg);
            end
    ...
    methods
        ...
        function set.avgnum(obj,val)
            if isempty(val) || val <= 0 || ceil(val) ~=val
                error('SpectrumAnalyzer:SetAverage','average number value should be positive integer!');
            end
            obj.avgnum = val;
        ...
        function set.startfreq(obj,val)
            if isempty(val) || ~isnumeric(val) || ~isreal(val) || val <= 0
                error('SpectrumAnalyzer:SetError','Invalid frequency value.');
            end
        ...
        function set.trigmod(obj,val)
            switch val
                ...
                otherwise
                    error('SpectrumAnalyzer:SetTrigMode','Invalid trig mode.')
            end
        end
        function val = get.trigmod(obj)
            ...
            switch resp
                ...
                case {'EXT1','EXT2'}
                    val = 4;
                otherwise
                    error('SpectrumAnalyzer:GetTrigModMode','query instrument failed.')
            end
        end
        function set.extref(obj,val)
            switch val
                ...
                otherwise
                    error('SpectrumAnalyzer:SetExtRefMode','Invalid trig mode.')
            end
        end
        function val = get.extref(obj)
            ...
            switch resp
                ...
                case 'EXT'
                    val = 2;
                otherwise
                    error('SpectrumAnalyzer:GetExtRefMode','query instrument failed.')
            end
        end
        function set.reflevel(obj,val)
            if isempty(val) || ~isnumeric(val) || ~isreal(val) || val < -150 || val > 30
                error('SpectrumAnalyzer:SetError','Invalid reference level value.');
            end
        ...
        function set.bandwidth(obj,val)
            if isempty(val) || ~isnumeric(val) || ~isreal(val) || val <= 0
                error('SpectrumAnalyzer:SetError','Invalid bandwith value.');
        ...
        function set.numpts(obj,val)
            if isempty(val) || val <= 0 || ceil(val) ~=val
                error('SpectrumAnalyzer:SetNumPts','numpts value should be positive integer!');
            end
        ...
        function set.on(obj,val)
            if isempty(val)
                error('SpectrumAnalyzer:SetOnOff', 'value of ''on'' must be a bolean.');
            end
            if ~islogical(val)
                if val == 0 || val == 1
                    val = logical(val);
            ...
            if val
                ...
                error('SpectrumAnalyzer:SetOnOff', 'off not implemeted.');
            end
            obj.on = val;
        end
